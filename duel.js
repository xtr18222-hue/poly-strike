...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory();else root.PolyDuel=factory();})(typeof globalThis!=='undefined'?globalThis:this,function(){'use strict';
 ...
 function create(map,C){
  ...
  function player(s,id){return {id,name:'Operator '+(id+1),x:s.x,y:1.7,z:s.z,yaw:id?Math.PI:0,pitch:0,hp:100,alive:true,weapon:'ak47',primary:'ak47',primaryLocked:false,dropped:false,ammo:Object.fromEntries(Object.entries(C.WEAPONS).map(([k,w])=>[k,{mag:w.mag,reserve:w.reserve}])),reload:0,reloadKey:null,kills:0,deaths:0,shotsFired:0,shotsHit:0};}
  const state={mapId:map.id||'sandline',phase:'buy',round:1,score:[0,0],buyClock:5,roundClock:90,endClock:0,time:0,lastWinner:null,matchWinner:null,players:spawns.map(player),events:[],drops:[]};
  function step(dt){
   ...
   else if(state.phase==='end'){state.endClock=Math.max(0,state.endClock-dt);if(state.endClock<1e-8){if(state.score.some(s=>s>=5)){state.phase='matchover';state.matchWinner=state.score[0]>=5?0:1;}else{state.phase='buy';state.round++;state.drops=[];state.events=[];state.buyClock=5;state.roundClock=90;state.lastWinner=null;state.players=spawns.map((s,id)=>({...player(s,id),name:state.players[id].name,kills:state.players[id].kills,deaths:state.players[id].deaths,shotsFired:state.players[id].shotsFired,shotsHit:state.players[id].shotsHit,primary:state.players[id].primary,primaryLocked:state.players[id].primaryLocked,weapon:state.players[id].primary}));nextFire.fill(0);movement.forEach(m=>{m.at=state.time;m.budget=1;m.vertical=1;});}}}
  }
  ...
  function reload(id,key){if((id!==0&&id!==1)||!Object.hasOwn(C.WEAPONS,key)||!['buy','live'].includes(state.phase))return false;const p=state.players[id],w=C.WEAPONS[key],a=p.ammo[key];if(!p.alive||!owns(p,key)||p.reload>0||!w.mag||a.mag>=w.mag||a.reserve<=0)return false;p.reload=w.reloadTime;p.reloadKey=key;return true;}
  const owns=(p,key)=>key==='deagle'||key==='knife'||key===p.primary&&!p.dropped;
  ...
  function shoot(id,s){
   ...
   if((id!==0&&id!==1)||!p||!s||!w||state.phase!=='live'||!p.alive||!owns(p,s.weapon)||p.reload>0||state.time+1e-8<nextFire[id]||!Number.isSafeInteger(s.seq)||s.seq<0||s.seq<=lastSeq[id]||!vector(s.origin)||!vector(s.dir))return reject();
   ...
   lastSeq[id]=s.seq;nextFire[id]=state.time+w.fireInterval;p.weapon=s.weapon;if(w.mag){a.mag--;p.shotsFired++;}
   ...
   result.dmg=C.shotDamage(s.weapon,part,distance);result.target=1-id;result.part=part;
   if(w.mag)p.shotsHit++;
  ...
  function move(id,s){
   ...
   if(!p.primaryLocked&&['ak47','awp','kar98'].includes(s.primary)){p.primary=s.primary;p.primaryLocked=true;if(!owns(p,p.weapon))p.weapon=p.primary;}
   if(typeof s.weapon==='string'&&owns(p,s.weapon))p.weapon=s.weapon;
   return true;
  }
  let dropSeq=0;
  function drop(id){
   const p=state.players[id];if(![0,1].includes(id)||!p||state.phase!=='live'||!p.alive||p.dropped||state.drops.length>=8)return false;
   const a=p.ammo[p.primary];state.drops.push({id:'drop-'+(++dropSeq),key:p.primary,x:p.x,z:p.z,mag:a.mag,reserve:a.reserve});
   p.dropped=true;p.weapon='deagle';p.reload=0;p.reloadKey=null;return true;
  }
  function pickup(id,dropId){
   const p=state.players[id],i=state.drops.findIndex(d=>d.id===dropId),d=state.drops[i];
   if(![0,1].includes(id)||!p||!d||state.phase!=='live'||!p.alive||!p.dropped||d.key!==p.primary||Math.hypot(p.x-d.x,p.z-d.z)>2.5||!C.segmentClear(p,d,expanded))return false;
   p.ammo[p.primary]={mag:d.mag,reserve:d.reserve};p.dropped=false;p.weapon=p.primary;p.reload=0;p.reloadKey=null;state.drops.splice(i,1);return true;
  }
  return {state,step,shoot,move,reload,drop,pickup,snapshot:()=>JSON.parse(JSON.stringify(state))};
 }
 return {create};
});
