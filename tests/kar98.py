...
with sync_playwright() as p:
    ...
    check(page.evaluate('Object.keys(Game.state().ammo).length') == 7, 'ammo registry has 7 weapons')
    ...
    for w, dur in [('mosin', 3.8), ('deagle', 2.6), ('bayonet', 1.5)]:
    ...
    check(v1 != v0, 'bayonet inspect variant alternates (%d -> %d)' % (v0, v1))
    check(v2 == v0, 'bayonet has exactly two variants (%d -> %d)' % (v1, v2))
    ...
    def body_outcome(px, pz):
        ...
        page.wait_for_timeout(250)
        st = page.evaluate('({mag: Game.state().ammo.mosin.mag, alive: Game.state().alive})')
        return st
