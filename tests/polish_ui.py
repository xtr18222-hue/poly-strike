...
with sync_playwright() as p:
 ...
 page.locator('#username').fill('<XTR & tester>');page.locator('#username').dispatch_event('change');page.reload();page.wait_for_function('window.PolyAsset && window.PolyAsset.progress().soldier',timeout=90000);page.wait_for_function('window.Game',timeout=90000);assert page.locator('#username').input_value()=='<XTR & tester>'
