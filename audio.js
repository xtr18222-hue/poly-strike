...
window.PolyAudio = (() => {
  ...
  function sound(type) { if (!ctx || muted || ctx.state !== 'running') return; try {
    ...
    const cues={magout:[340,.10,.13],magin:[640,.085,.16],bolt:[1350,.075,.12],dry:[1900,.035,.07],heartbeat:[58,.18,.18],enemyStep:[115,.10,.11]};
    if(cues[type]){const [hz,dur,level]=cues[type];tone(hz,dur,level,type==='heartbeat'?'sine':'triangle');if(type==='heartbeat')tone(65,.12,.10,'sine',.21);return;}
